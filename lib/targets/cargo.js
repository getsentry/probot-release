 ...
 *
 * @param {Context} context Enhanced Github context
 * @param {string} directory Path to the root crate / package
 * @returns {Promise<MetaData>} An object containing cargo metadata
 * @async
 */
async function getCargoMetadata(context, directory) {
  ...
  const json = await spawn(CARGO_BIN, args, undefined, logger);
...
async function publishPackage(context, crate) {
  ...
  return shouldPerform() && spawn(CARGO_BIN, args, undefined, logger);
}
...
async function publishWorkspace(context, directory) {
  const metadata = await getCargoMetadata(context, directory);
