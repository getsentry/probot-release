...
const { forEachFile, withTempDir } = require('./files');
const { isSorted, shouldPerform } = require('./utils');
...
async function createRelease(context, tag) {
  const changelog = await getFile(context, CHANGELOG_PATH);
...
module.exports = (robot) => {
  ...
  robot.on('delete', async (context) => {
    ...
    await removeTag(context, context.payload.ref, config);
  });
