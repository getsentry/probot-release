...
async function performRelease(context, tag, config) {
  ...
  await withTempDir(async downloadDirectory => {
    ...
    const store = storeFn(
      context.repo({ sha: tag.sha }),
      downloadDirectory,
      logger
    );
    await Promise.all(
      config.targets.map(target => releaseTarget(target, context, tag, store))
    );
  });
}
...
async function processTag(context, tag, config) {
  ...
  if (latestStatuses.length === 0) {
    ...
    logger.info(
      `Skipping release of ${owner}/${repo}:${
        tag.ref
      } as no status checks were found`
    );
    return;
  }
  ...
  if (config.targets.length === 0) {
    ...
    logger.info(
      `Skipping release of ${owner}/${repo}:${
        tag.ref
      } since no targets were configured`
    );
    return;
  }
  ...
  logger.info(
    `Scheduling release of ${id} in ${RELEASE_TIMEOUT / 1000} seconds`
  );
