...
const { listFiles, withTempDir } = require('./files');
const { cloneContext, isSorted } = require('./utils');
...
async function getStatuses(context, ref) {
  ...
  const statuses = await context.github.paginate(response, result => result.data);
 ...
 * context, as well as the list of files to release. It can then decide which
 * files will be included in the release.
 *
 * @param {object | string} target Target name or configuration
 * @param {string[]} files A list of paths to
 * @returns {Promise} A promise that resolves when the release has succeeded
 * @async
 */
async function releaseTarget(target, context, tag, files) {
  const config = (typeof target === 'string') ? { name: target } : target;
  const targetFn = tryRequire(`./targets/${config.name}`, require);
  if (targetFn == null) {
    logger.warn(`Skipping unknown deploy target "${config.name}"`);
    return null;
  }

  const targetContext = cloneContext(context, { tag, logger, ...config });
  return targetFn(targetContext, files);
}
 ...
 * completely. Uses a local temp directory to store the assets, and deletes it
...
async function processTag(context, tag, config) {
  ...
  if (latestStatuses.length === 0) {
    ...
    logger.info(`Skipping release of ${owner}/${repo}:${tag.ref} as no status checks were found`);
    return;
  }

  if (latestStatuses.some(status => status.state === STATE_PENDING)) {
    // Checks are still running, so no reason to proceed
    logger.info(`Skipping release of ${owner}/${repo}:${tag.ref} as status checks are pending`);
    return;
  }
  ...
  if (config.targets.length === 0) {
    ...
    logger.info(`Skipping release of ${owner}/${repo}:${tag.ref} since no targets were configured`);
    return;
  }
