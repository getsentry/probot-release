...
async function getTags(context) {
  ...
  tagCache[key] = context.github.paginate(
    context.github.repos.getTags(repo, { per_page: 100 }),
    response => response.data.map(tag => ({ ref: tag.tag, sha: tag.commit.sha })),
  );
...
async function createRelease(context, tag) {
  ...
  console.info(`Creating release ${params.owner}/${params.repo}:${tag}`);
...
function downloadS3Directory(remoteDir, localDir) {
  ...
  console.info(`Downloading S3 bucket ${bucket}:${remoteDir} to ${localDir}`);
...
function uploadReleaseAssets(context, release, directory) {
  console.info(`Uploading release assets from ${directory}`);
  return forEachFile(directory, (path, name) => {
    ...
    console.info(`Uploading asset "${name}" to ${params.owner}/${params.repo}:${release.tag_name}`);
...
async function processTag(context, tag) {
  const repo = context.repo();
  console.info(`Processing tag ${repo.owner}/${repo.repo}:${tag.ref} (${tag.sha})`);
  ...
  if (latestStatuses.some(status => status.state !== STATE_SUCCESS)) {
    ...
    console.info(`Skipping release of ${repo.owner}/${repo.repo}:${tag.ref} as status checks have failed`);
    return;
  }

  // All checks have cleared, we're ready to release now
  await withTempDir(async (downloadDirectory) => {
    const commitDirectory = `${repo.owner}/${repo.repo}/${tag.sha}`;
    ...
    console.info(`Release completed: ${release.html_url}`);
  });
}
