 ...
 * @async
 */
async function releaseTarget(target, context, tag, store) {
  ...
  try {
    const targetContext = cloneContext(context, { ...config, tag, logger, store });
...
async function performRelease(context, tag, config) {
  ...
  await withTempDir(async (downloadDirectory) => {
    const storeFn = tryRequire(`./stores/${config.store}`, require);
    if (storeFn == null) {
      logger.error(`Unknown store "${config.store}"`);
      return;
    }

    const store = storeFn(context.repo({ sha: tag.sha }), downloadDirectory, logger);
    await Promise.all(config.targets.map(target => releaseTarget(target, context, tag, store)));
  });
}
