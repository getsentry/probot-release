 ...
 */
function exitCodeError(code, command, args, options) {
...
/**
 * Strips env values from the options array
 *
 * @param {string[]} args Optional arguments to pass to the command
 */
function convertOptionsForError(options) {
  ...
  if (options && options.env) {
    envVars = { envVars: Object.keys(options.env) };
    delete options.env;
  }
  return JSON.stringify(Object.assign({}, options, envVars));
}
...
function spawn(command, args, options) {
  return new Promise((resolve, reject) => {
    try {
      ...
      process.on('exit', code => (code === 0 ? resolve() : reject(exitCodeError(code, command, args, options))));
      process.on('error', error => {
        reject(exitCodeError(error.code, command, args, options));
      });
    } catch (error) {
      reject(exitCodeError(error.code, command, args, options));
    }
