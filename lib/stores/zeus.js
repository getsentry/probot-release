...
module.exports = (commit, downloadDirectory, logger = console) => {
  ...
  function downloadFile(file) {
    ...
    const stream = request
      .get(url, { auth: { bearer: ZEUS_API_TOKEN } })
  ...
  function listFiles() {
    ...
    const url = `${ZEUS_SERVER_URL}/api/repos/gh/${owner}/${repo}/revisions/${
      sha
    }/artifacts`;
    const auth = { bearer: ZEUS_API_TOKEN };
    logger.debug(
      `Loading list of Zeus artifacts for ${owner}/${repo}/revisions/${sha}`
    );
