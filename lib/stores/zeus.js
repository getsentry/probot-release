...
module.exports = (commit, downloadDirectory, logger = console) => {
  ...
  function downloadFile(file) {
    const cached = downloadCache[file.download_url];
    if (cached) {
      return cached;
    }

    const url = `${ZEUS_SERVER_URL}/${file.url}`;
    logger.debug(`Downloading Zeus file ${url} to ${downloadDirectory}`);

    const localFile = join(downloadDirectory, file.name);
    const stream = request.get(url, { auth: { bearer: ZEUS_API_TOKEN } })
    ...
    downloadCache[file.download_url] = promise;
    return promise;
  }
  ...
  function listFiles() {
    ...
    const url = `${ZEUS_SERVER_URL}/api/repos/gh/${owner}/${repo}/revisions/${sha}/artifacts`;
    const auth = { bearer: ZEUS_API_TOKEN };
    logger.debug(`Loading list of Zeus artifacts for ${owner}/${repo}/revisions/${sha}`);
