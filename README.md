...
target key is empty, the bot defaults to the `"github"` target:
...
generated by `npm pack` in the release assets. The file will be uploaded to the
...
from the Github repository with the tag that is being released. No release
...
The default store is Amazon S3 and can be changed via the `store` parameter in
...
Download artifacts from an Amazon S3 bucket. To create a bucket, please refer to
the
[official instructions](http://docs.aws.amazon.com/AmazonS3/latest/gsg/CreatingABucket.html).
...
repositories. Also, each repository must be activated and a hook configured in
the repository settings.

To upload releases, use the [zeus-ci](https://npmjs.org/package/zeus-ci) command
line utility.
